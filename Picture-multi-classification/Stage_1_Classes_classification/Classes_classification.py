...
def train_model(model, criterion, optimizer, scheduler, num_epochs=100):
    ...
    torch.save(model.state_dict(), 'best_model' + str(t.month) + str(t.day) + str(t.hour) + str(t.minute) + '.pt')
    print('Best val classes Acc: {:.2%}'.format(best_acc))
    return model, Loss_list, Accuracy_list_classes


network = Net().to(device)
# network2 = torch.load('best_model.pt')
optimizer = optim.SGD(network.parameters(), lr=0.02, momentum=0.7)
